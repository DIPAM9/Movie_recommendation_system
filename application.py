...
def recommend():
    ...
    suggestion_list, genre_list = suggest(movie_name=movie_name)
    df = pd.DataFrame(list(zip(suggestion_list, genre_list)), columns=['Movie Name', 'Genre'])
...
def suggest(movie_name):
    ...
    return recommend_movie_list, genre_list
